...
(function (l11_1) {
    class Flower {
        ...
        draw() {
            ...
            l11_1.context.fillRect(this.position[0] - this.dimensions[0] / 2, this.position[1], this.dimensions[0], -this.dimensions[1]);
            l11_1.context.beginPath();
            l11_1.context.moveTo(this.innerPoints[0][0], this.innerPoints[0][1]);
            for (let i = 0; i < this.innerPoints.length; i++) {
                l11_1.context.bezierCurveTo(this.outerPoints[i][0], this.outerPoints[i][1], this.outerPoints[(i + 1) % this.innerPoints.length][0], this.outerPoints[(i + 1) % this.innerPoints.length][1], this.innerPoints[(i + 1) % this.innerPoints.length][0], this.innerPoints[(i + 1) % this.innerPoints.length][1]);
            }
            l11_1.context.closePath();
            l11_1.context.fillStyle = this.color;
            l11_1.context.fill();
            l11_1.context.strokeStyle = "black";
            l11_1.context.stroke();
            l11_1.context.fillStyle = "hsl(40, 100%, " + this.nektar + "%)";
            l11_1.context.beginPath();
            l11_1.context.arc(this.position[0], this.position[1] + -this.dimensions[1], this.size / 5, 0, Math.PI * 2);
            l11_1.context.fill();
            if (this.nektar < 100) {
                this.nektar += 0.5;
            }
